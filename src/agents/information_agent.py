...
def _mock_retention_metrics(time_range: str) -> Dict:
    base  = 87.3
    delta = random.uniform(-6.0, 1.0)
    grr   = round(base + delta, 2)
    return {
        "gross_retention_rate": grr,
        "gross_retention_prev": round(base, 2),
        "net_retention_rate":   round(107.4 + random.uniform(-6, 2), 2),
        "churn_rate":           round(100 - grr, 2),
        "churned_accounts":     random.randint(18, 40),
        "total_accounts":       412,
        "at_risk_accounts":     random.randint(15, 45),
        "time_range":           time_range or "last_month",
        "breakdown": {
            "Enterprise": round(93.1 + random.uniform(-2, 1), 2),
            "Mid-Market": round(88.7 + random.uniform(-4, 1), 2),
            "SMB":        round(79.2 + random.uniform(-6, 1), 2),
        },
    }
...
def _mock_bookings_metrics(time_range: str) -> Dict:
    return {
        ...
        "bookings_vs_target_pct": round(random.uniform(-10, 12), 1),
        "time_range":             time_range or "last_month",
    }

def _mock_cac_metrics(time_range: str) -> Dict:
    return {
        "blended_cac_usd":       round(random.uniform(8200, 11500), 0),
        "paid_cac_usd":          round(random.uniform(14000, 19000), 0),
        "organic_cac_usd":       round(random.uniform(3500, 6000), 0),
        "payback_period_months": round(random.uniform(13, 24), 1),
        "cac_vs_prev_pct":       round(random.uniform(-5, 18), 1),
        "time_range":            time_range or "last_month",
    }
...
class InformationAgent(BaseAgent):
    """
    Fetches structured metrics from Databricks or mock
    generators for registered enterprise data products.
    """
    ...
    def _execute(self, request: AgentRequest) -> AgentResult:
        products   = request.data_products or \
                     self._detect_products(request.query)
        ...
        for product in products:
            if self.enable_mock or not self._connector:
                ...
                sources.append(
                    f"[MOCK] "
                    f"{DATA_PRODUCTS.get(product,{}).get('table', product)}"
                )
            else:
                metrics = self._fetch_live(product, time_range)
                sources.append(
                    DATA_PRODUCTS.get(product, {}).get("table", product)
                )

            all_metrics[product] = metrics
            anomalies.extend(
                self._detect_anomalies(product, metrics)
            )
        ...
        return AgentResult(
            ...
            summary    = summary,
            sources    = sources,
            confidence = 0.85 if self.enable_mock else 0.95,
            metadata   = {"products_queried": products,
                          "time_range": time_range},
        )

    def _fetch_mock(self, product: str,
                    time_range: str) -> Dict:
        generator = MOCK_GENERATORS.get(product)
        return generator(time_range) if generator \
               else {"error": f"No mock data for {product}"}

    def _fetch_live(self, product: str,
                    time_range: str) -> Dict:
    ...
    def _detect_anomalies(self, product: str,
        ...
        if product == "retention":
            ...
            if at_risk > 30:
                anomalies.append(
                    f"⚠️ {at_risk} accounts at-risk — "
                    f"CS outreach required."
                )
        if product == "cac":
            payback = metrics.get("payback_period_months", 0)
            if payback > 20:
                anomalies.append(
                    f"⚠️ CAC payback ({payback}mo) exceeds "
                    f"20-month target."
                )
        if product == "bookings":
            ...
            if vs_t < -5:
                anomalies.append(
                    f"⚠️ Bookings {abs(vs_t)}% below target."
                )
        return anomalies

    def _build_summary(self, all_metrics: Dict,
                        anomalies: List[str],
                        time_range: str) -> str:
